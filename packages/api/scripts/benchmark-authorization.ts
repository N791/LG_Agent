...
async function main() {
  ...
  if (!selected) throw new Error('Authorization benchmark requires at least one role assignment.');
  ...
  await Promise.all(
    Array.from({ length: concurrency }, async (_, worker) => {
      for (let index = worker; index < iterations; index += concurrency) {
        ...
        durations.push(performance.now() - queryStartedAt);
      }
    }),
  );
  stopSampling = true;
  ...
  const maxConnections = await prisma.$queryRaw<{ max_connections: number }[]>`
  ...
  if (
    result.latencyMs.p95 > thresholds.p95Ms ||
...
async function sampleConnections(stop: () => boolean, observe: (value: number) => void) {
  while (!stop()) {
    const rows = await prisma.$queryRaw<{ connections: number }[]>`
