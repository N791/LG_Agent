...
export const TaskCard: React.FC<TaskCardProps> = ({ task, onClick }) => {
  ...
  return (
    <Card
      ...
      extra={
        <Tag
          color={task.taskType === TaskType.MANDATORY ? 'red' : 'blue'}
    ...
    >
      <div className="text-gray-500 mb-4 h-10 line-clamp-2 text-sm">
        {task.summary ?? task.description ?? t('taskCard.noDesc')}
      </div>
      <div className="flex justify-between items-center mt-auto pt-4 border-t border-gray-50">
        <span className="text-xs font-medium text-purple-600 bg-purple-50 px-2 py-1 rounded-md">
          {t('progress.stage')} {task.stage}
        </span>
        <span className="text-xs font-medium text-gray-500">{task.difficulty}</span>
      </div>
    </Card>
  );
};
