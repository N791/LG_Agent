  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setLogs(
          (logsRes as unknown as { data?: Record<string, unknown>[] }).data ??
            (logsRes as unknown as Record<string, unknown>[]),
        );
        setMetrics(
          (metricsRes as unknown as { data?: Record<string, unknown>[] }).data ??
            (metricsRes as unknown as Record<string, unknown>[]),
        );
        setAuditLogs(
          (auditRes as unknown as { data?: Record<string, unknown>[] }).data ??
  ...
  const metricColumns = [
    ...
    },
    {
      ...
      render: (rating: string) =>
        rating ? <Tag color={getRatingColor(rating)}>{rating}</Tag> : '-',
      width: 150,
    },
    {
      title: 'Path',
